...
import { CaregiverDetails, Caregivers } from "../types/CaregiverDetailTypes";
...
const post = async (
  ...
  try {
    ...
    const { data } = await baseAPIClient.post("/caregiver", myData, {
...
const deleteCaregiver = async (caregiverId: number): Promise<void> => {
  ...
  try {
    const response = await baseAPIClient.delete(`/caregiver/${caregiverId}`, {
      headers: { Authorization: bearerToken },
    });
